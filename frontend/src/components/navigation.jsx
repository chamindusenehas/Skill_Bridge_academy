            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                >
                  {link.name}
                  {link.submenu && (
                    <ChevronDown className={cn('w-4 h-4 transition-transform duration-200', activeSubmenu === link.name && 'rotate-180')} />
                  )}
                </Link>
                <AnimatePresence>
                    ...
                    >
                      <div className="bg-card border border-border rounded-xl p-2 shadow-xl shadow-primary/10">
                        {link.submenu.map((sublink, idx) => (
                          ...
                          >
                            <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}>
                              {sublink.name}
                            </motion.span>
                          </Link>
                        ))}
          ...
          {isMobileMenuOpen && (
            ...
            >
              <div className="py-4 space-y-1">
                ...
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: navLinks.length * 0.1 }} className="pt-4 px-4 flex flex-col gap-2">
                  {user ? (
                    <Link to="/dashboard" className="w-full">
                      <Button className="w-full font-medium bg-gradient-to-r from-primary to-accent">Dashboard</Button>
                    </Link>
                  ) : (
                    <>
